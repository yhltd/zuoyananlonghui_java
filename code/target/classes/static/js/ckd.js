...
function printOutboundOrder() {
            ...
            <style>
                ...
                * {
                ...
                .print-table th {
                 ...
                 /* 新增：序号列固定宽度样式 */
                    .print-table th.index-column,
                    .print-table td.index-column {
    ...
    selectedColumns.forEach(function(column, index) {
        // 如果是序号列，添加特定class
        if (column.type === 'index') {
            tableHeaders += '<th class="index-column">' + column.name + '</th>';
        } else {
    ...
    var printContent = `
        ...
        <head>
            ...
            <style>
                ...
                @page {
                    size: 241mm 279.4mm !important;  /* 使用!important确保优先级 */
                    margin: 0mm 0mm;  /* 将边距设置为0 */
                }
                ...
                * {
                ...
                body {
                    ...
                    color: black;
                    width: 227mm;
                    height: 279mm;  /* 重要：确保body高度与@page高度一致 */
                    margin: 0 auto;
                    line-height: 1.2;
                ...
                .page-container {
                    position: relative;
                    width: 227mm;
                    height: 279mm;  /* 确保容器高度与纸张高度一致 */
                    overflow: hidden;  /* 隐藏溢出内容 */
                    page-break-inside: avoid;  /* 避免容器内部分页 */
                }
    ...
    function generatePageContent(pageData, copyTag, isFirstPage) {
        ...
        return `
            <div class="page-container">
                ...
                <div class="total-row">
                    <div style="margin-left: 3px;"><strong>合计数量：</strong>${totalQuantity}</div>
                    <div><strong>合计金额(大写)：</strong>${chineseAmount}</div>
                    <div style="margin-right: 3px;"><strong>合计金额：</strong>${totalAmount.toFixed(2)} 元</div>
                </div>
                ...
                <div class="footer">
                    <div class="footer-top">
                        <div style="margin-left: 3px;"><strong>制单人：</strong>${$('#company-address1').val() || ''}</div>
                        <div><strong>审核人：</strong>${$('#company-phone1').val() || ''}</div>
                        <div><strong>送货人：</strong>${$('#songhuoren').val() || ''}</div>
                        <div><strong>收货人：</strong>${$('#shouhuoren').val() || ''}</div>
                    </div>
                    <div class="footer-bottom">
                        <div style="margin-left: 3px;"><strong>地址：</strong>四川省成都市新都区石板滩街道石木路588号</div>
                        <div style="margin-right: 3px;"><strong>联系电话：</strong>13730601502</div>
                    </div>
                </div>
