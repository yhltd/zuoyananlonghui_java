...
function loadEmployeeList() {
    ...
    $ajax({
        ...
        if (res.code === 200) {
            ...
            var $select = $('#dengjiyuan');
            $select.empty(); // 清空现有选项
            $select.append('<option value="">请选择登记员</option>');

            if (res.data && Array.isArray(res.data)) {
                // 遍历数据，从m字段获取员工姓名
                res.data.forEach(function(item) {
                    // 安全地获取m字段的值
                    var employeeName = item && item.aas;
                ...
                $select.append('<option value="">暂无登记员数据</option>');
            }
        } else {
            console.error('获取员工列表失败:', res.msg);
            $('#dengjiyuan').html('<option value="">加载失败</option>');
        }
    });
}
