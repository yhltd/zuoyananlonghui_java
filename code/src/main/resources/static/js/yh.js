...
$(function () {
    ...
    $('#update-btn').click(function () {
        let rows = getTableSelection('#yhTable');
        ...
        $('#update-xssr').val(rows[0].data.xssr);
        $('#update-yssr').val(rows[0].data.yssr);
        $('#update-zxsr').val(rows[0].data.zxsr);
        $('#update-rkzc').val(rows[0].data.rkzc);
        $('#update-fyzc').val(rows[0].data.fyzc);
    });
...
function setTable(data) {
    if ($('#yhTable').html != '') {
        $('#yhTable').bootstrapTable('load', data);
    }
