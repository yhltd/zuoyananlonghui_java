...
//新增弹窗里点击关闭按钮
    ...
    $("#add-submit-btn").click(function () {
        ...
        let params = {
            ...
            d: $('#add-d').val(),              // 合同号
            e: $('#add-e').val(),              // 任务号
            zhuangtai: $('#add-zhuangtai').val() || '未创建', // 工艺规程状态，默认未创建
            g: $('#add-g').val(),              // 工序
            h: $('#add-h').val(),              // 名称
            i: $('#add-i').val(),              // 规格型号
            j: $('#add-j').val(),              // 单位
            ...
            aw: $('#add-aw').val(),            // 重量
            ax: $('#add-ax').val(),            // 零件尺寸
            shijijiaohuoriqi: $('#add-shijijiaohuoriqi').val(), // 订单实际交货日期
            ay: $('#add-ay').val(),            // 订单要求交货日期
            at: $('#add-at').val(),            // 备注
            dengjiriqi: $('#add-dengjiriqi').val() || getCurrentDate() // 登记日期（默认当前日期）
        };
    ...
    $('#update-modal').on('shown.bs.modal', function () {
        if (window.selectedUserData) {
            ...
            $('#update-zhuangtai').val(selectedRow.zhuangtai || '');  // 工艺规程状态
            $('#update-i').val(selectedRow.i || '');  // 规格型号
            ...
            $('#update-aw').val(selectedRow.aw || '');  // 重量
            $('#update-ax').val(selectedRow.ax || '');  // 零件尺寸
            $('#update-shijijiaohuoriqi').val(selectedRow.shijijiaohuoriqi || '');  // 订单实际交货日期
            $('#update-ay').val(selectedRow.ay || '');  // 订单要求交货日期
            $('#update-at').val(selectedRow.at || '');  // 备注
            $('#update-dengjiriqi').val(selectedRow.dengjiriqi || getCurrentDate());  // 登记日期

            $('#id').val(selectedRow.id || '');  // 主键ID
    ...
    $('#update-submit-btn').click(function () {
        ...
        if (msg) {
            ...
            let params = {
                ...
                e: $('#update-e').val(),  // 任务号
                h: $('#update-h').val(),  // 名称
                zhuangtai: $('#update-zhuangtai').val(),  // 工艺规程状态
                i: $('#update-i').val(),  // 规格型号
                j: $('#update-j').val(),  // 单位
                k: $('#update-k').val(),  // 数量
                l: $('#update-l').val(),  // 材质
                ...
                n: $('#update-n').val(),  // 金额
                aw: $('#update-aw').val(),  // 重量
                ax: $('#update-ax').val(),  // 零件尺寸
                shijijiaohuoriqi: $('#update-shijijiaohuoriqi').val(),  // 订单实际交货日期
                ay: $('#update-ay').val(),  // 订单要求交货日期
                at: $('#update-at').val(),  // 备注
                dengjiriqi: $('#update-dengjiriqi').val()  // 登记日期
            };
...
function deleteExistingReturnOrder(returnNo, callback) {
    ...
    $ajax({
        type: 'post',
        url: '/xhtjl/deleteReturnOrderByNo',
        data: {
            returnNo: returnNo
        },
        dataType: 'json'
...
function exportToExcel(filename) {
    ...
    $ajax({
        type: 'post',
        url: '/xhtjl/getListExcludeThjl', // 需要后端提供获取所有数据的接口
        contentType: 'application/json',
        data: JSON.stringify({
            pageNum: 1,
            pageSize: 9999999,
    ...
    try {
        ...
        var excelData = data.map(function(item) {
            return {
                ...
                '重量': item.aw || '',
                '零件尺寸': item.ax || '',
                '订单实际交货日期': item.shijijiaohuoriqi || '',
                '订单要求交货日期': item.ay || '',
                '合同': item.d || '',
                '备注': item.at || ''
            };
