...
@Mapper
@Repository
public interface YssjMapper extends BaseMapper<Yssj> {

    @Select("select riqi,dh,shdw,mc,mh,gg,js,zl,dj,je,sfhs from xiaoshoudan")
    List<Yssj> getList();

    @Select("select riqi,dh,shdw,mc,mh,gg,js,zl,dj,je,sfhs from xiaoshoudan where riqi >= convert(date,#{ksrq}) and riqi <= convert(date,#{jsrq})")
