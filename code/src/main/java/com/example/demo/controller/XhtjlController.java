...
@Slf4j
@RestController
@RequestMapping("/xhtjl")
public class XhtjlController {
    @Autowired
    private XhtjlService xhtjlService;
    ...
    public Result<?> updateField(@RequestBody Map<String, Object> params) {
        try {
            ...
            boolean success = xhtjlService.updateField(id, params);
     ...
     */
    @RequestMapping("/add")
    public ResultInfo add(@RequestBody Xhtjl htjl, HttpSession session) {
        try {

            // 插入新用户数据
            boolean result = xhtjlService.add(htjl);
    ...
    public ResultInfo delete(@RequestBody HashMap map, HttpSession session) {
        ...
        try {
            for (int i = 0; i < idList.size(); i++) {
                ...
                xhtjlService.delete(Collections.singletonList(this_id));
            }
    ...
    public ResultInfo queryList(String name, String department, HttpSession session) {
        try {
            List<Xhtjl> list = xhtjlService.queryList(name, department);
     ...
     */
    @RequestMapping("/save")
    public ResultInfo save(@RequestBody Xhtjl htjl, HttpSession session) {
        try {

            // 直接插入数据
            boolean result = xhtjlService.save(htjl);
            System.out.println("插入结果: " + result);
    ...
    public ResultInfo getddh(HttpSession session) {
        try {
            String getList = xhtjlService.getddh();
    ...
    public ResultInfo save1(@RequestBody Xhtjl htjl, HttpSession session) {
        try {

            // 直接插入数据
            boolean result = xhtjlService.save1(htjl);
            System.out.println("插入结果: " + result);
    ...
    public ResultInfo getddh1(HttpSession session) {
        try {
            String getList = xhtjlService.getddh1();
            return ResultInfo.success("获取成功", getList);
        } catch (Exception e) {
            e.printStackTrace();
            log.error("获取失败：{}", e.getMessage());
    ...
    public ResultInfo getById(String id, HttpSession session) {
        try {
            Xhtjl data = xhtjlService.getById(id);
            if (data != null) {
    ...
    public ResultInfo getByIds(@RequestParam List<String> ids, HttpSession session) {
        try {
            List<Xhtjl> dataList = xhtjlService.getByIds(ids);
    ...
    public Result importExcel(@RequestBody Map<String, Object> request, HttpSession session) {
        try {
            ...
            // 调用Service进行数据导入
            Map<String, Object> importResult = xhtjlService.importExcelData(records);
    ...
    public ResultInfo getCustomerList(HttpSession session) {
        try {
            List<Xhtjl> getList = xhtjlService.getCustomerList();
            return ResultInfo.success("获取成功", getList);
        } catch (Exception e) {
            e.printStackTrace();
            log.error("获取失败：{}", e.getMessage());
